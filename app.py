...
if st.session_state.page == "library":
    ...
    # 1. 파일이 바뀌면 이전 해설 초기화 및 사진 새로 읽기
    ...
    if st.session_state.current_image is not None:
        ...
        if st.session_state.current_explanation is None:
            if st.button("동국 튜터식 해설 보기"):
                with st.spinner("지문을 분석하고 압축 저장 중입니다..."):
                    try:
                        # [AI 분석]
                        ...
                        new_data = {
                            ...
                            "title": title if title else "새로운 해설",
                            "content": res.text,
                            "bookmarked": False,
                            "image_base64": img_data_url # 사진도 글자로 영구 저장
                        }
                        new_ref.set(new_data)
                        st.session_state.library.append(new_data)
                        st.success("✅ 라이브러리에 영구 저장되었습니다!")
